...
function VUHDO_determineDebuff(aUnit, aClassName)
	...
	if (tInfo["missbuff"] ~= nil	and (sIsMiBuColorsInFight or not InCombatLockdown())) then
	...
	if (VUHDO_shouldScanUnit(aUnit)) then
		...
		for tCnt = 1, 255 do
			tName, _, tIcon, tStacks, tType, tDuration, tExpiry, _, _, _, tSpellId = UnitDebuff(aUnit, tCnt, false);
			if (tIcon == nil) then
				break;
			end
			...
			if ((not sIsRemoveableOnly or tAbility ~= nil) and tChosen ~= 6) then --VUHDO_DEBUFF_TYPE_CUSTOM
				if (tAbility ~= "I" or aUnit == "player") then -- Nur aus Spieler selbst wirkbar?
					...
					if (tDebuff ~= nil and VUHDO_isDebuffRelevant(tName, aClassName)) then
